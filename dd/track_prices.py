...
def format_number(number):
    """Format a number as a Polish string (e.g., 717.67 -> '717,67')."""
    try:
        if not isinstance(number, (int, float)):
            raise ValueError(f"Invalid number type: {number}")
        return f"{number:,.2f}".replace(",", " ").replace(".", ",")
    except Exception as e:
        logger.error(f"Error formatting number: {number}, error: {e}")
        return "Błąd"

# Detect text from an image using Google Vision API
...
# Parse invoice text using xAI API
def parse_invoice_text(text, paid_status):
    ...
    try:
        ...
        logger.info(f"Raw response from xAI: {raw_response}")
        return json.loads(raw_response)
    except json.JSONDecodeError:
        logger.error("Error: xAI did not return valid JSON")
        return None
    except Exception as e:
        logger.error(f"Unexpected error during xAI API call: {e}")
        return None
...
# Update or append ingredient data in Google Sheets
            ...
            if abs(sheet_net_price - ingredient['net_price_per_unit']) < 0.01:
                ...
                sheet.update(
                    ...
                    values=[[
                        ...
                        ingredient['name'],
                        ingredient['unit'],
                        format_number(ingredient['net_price_per_unit']),
                        ingredient['vat_percent'],
                        format_number(ingredient['gross_price_per_unit']),
                        seller
                    ]]
                )
                return
...
# Sync invoice status between unpaid and paid sheets
def sync_invoice_status():
    ...
    if not unpaid_sheet or not paid_sheet:
        return
    ...
    for i, row in enumerate(unpaid_data, start=2):
        if row['Opłacona (T/N)'] == 'T':
            kwota_float = float(str(row['Kwota Całkowita (PLN)']).replace(',', '.'))
            rows_to_move.append((i, [
                row['Data Wystawienia'],
                row.get('Numer Faktury', ''),
                row['Sprzedawca'],
                format_number(kwota_float),
                row['Kategoria'],
    ...
    unpaid_data = unpaid_sheet.get_all_records()
    sorted_data = sorted(unpaid_data, key=lambda x: float(x['Dni do Zapłaty'].replace(',', '.')) if x['Dni do Zapłaty'].replace(',', '.').isdigit() else float('inf'))
...
if __name__ == "__main__":
    ...
    if not invoice_files:
        logger.info("No images found in the 'invoices' folder.")
        exit()
    ...
    if not json_data:
        logger.error("Failed to parse invoice data.")
        exit()
    ...
    if target_sheet:
        target_sheet.append_row([
            ...
            json_data['seller'],
            format_number(json_data['total']),
            json_data['category'],
            json_data['due_date'],
            json_data['paid'],
            alert or days_left or "Błąd daty"
        ])
    ...
    sync_invoice_status()
    logger.info("Data saved to the sheet!")
